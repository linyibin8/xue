...
- **发版**：build **202606280913 已发 TestFlight**（ASC state=VALID，挂到内部组 Fastlane OneClick Internal=4 active testers，What to Test 已设）。altool 上传卡死(已知问题)但二进制已成功投递，按兜底手动跑 configure_testflight.py+assign 完成挂组(ASC_APP_ID=6783612413, BETA_GROUP_ID=2a5e0639...)。真机相机/语音流只能真机验收。注：Mac 上累积了 9 个历史会话遗留的 altool 卡死僵尸进程，建议清理。
